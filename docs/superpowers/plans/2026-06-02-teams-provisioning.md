# Teams Provisioning Tab Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Teams Provisioning" tab that creates Microsoft Teams class or standard teams, populated from a year group or direct user search, with per-member owner assignment and a progress log.

**Architecture:** New `src/Tools/TeamsProvisioning.ps1` following the sidebar+right-panel+sub-tabs pattern (same as PasswordReset/GroupCopy). Two new Graph scopes in Auth.ps1. Team creation uses `Invoke-WebRequest` to capture the `Location` header from the async 202 response, then polls the operation URL until the team is provisioned before adding members sequentially. Demo mode is wired through `src/Demo.ps1` like all other tools.

**Tech Stack:** WPF/XAML in PowerShell 7, Microsoft Graph REST API v1.0, MSAL.PS, `System.Windows.Threading.DispatcherTimer`, `System.Collections.Concurrent.ConcurrentQueue`

---

### Task 1: Add new Graph scopes

**Files:**
- Modify: `src/Auth.ps1` (lines 62–67)

- [ ] **Step 1: Update `$Script:GraphScopes` in Auth.ps1**

Find the current array (lines 62–67) and replace with:
...
```powershell
git add src/Auth.ps1
git commit -m "feat: add Team.Create and TeamMember.ReadWrite.All scopes"
```

---

### Task 2: Wire new tab into MainWindow.ps1 and Start.ps1

**Files:**
- Modify: `src/MainWindow.ps1`
- Modify: `Start.ps1`

- [ ] **Step 1: Add TabItem to the MainWindow XAML**

In `src/MainWindow.ps1`, find the block around lines 252–257:
...
- [ ] **Step 2: Add TabTeams to the `$Script:MainUI` hashtable**

In `src/MainWindow.ps1`, find the `$Script:MainUI = @{` block (around line 500). After the `TabGroupCopy` line add:

```powershell
        TabTeams         = $window.FindName('TabTeams')
```

- [ ] **Step 3: Wire up the tool initialisation in Show-MainWindow**

In `src/MainWindow.ps1`, find the block that calls `Initialize-*Tool` (around lines 524–534). After the GroupCopy line add:
...
- [ ] **Step 4: Dot-source TeamsProvisioning.ps1 in Start.ps1**

In `Start.ps1`, after line 50 (`GroupCopy.ps1`):
...
```powershell
git add src/MainWindow.ps1 Start.ps1
git commit -m "feat: wire Teams Provisioning tab into shell"
```

---

### Task 3: Create TeamsProvisioning.ps1 — XAML, state, and UI wiring

**Files:**
- Create: `src/Tools/TeamsProvisioning.ps1`

- [ ] **Step 1: Create the file with state variables and helper functions**

Create `src/Tools/TeamsProvisioning.ps1` with this content:
...
- [ ] **Step 2: Append the XAML here-string to the file**

Append to `src/Tools/TeamsProvisioning.ps1`:
...
- [ ] **Step 3: Append `Initialize-TeamsProvisioningTool` to the file**
...
- [ ] **Step 4: Launch and verify the tab renders**

Run: `Launch.cmd`

Expected:
- App opens with no console errors
- "Teams Provisioning" tab visible in the tab bar
- Clicking it shows: sidebar with Team Name field, Team Type radios, Population radios, Selection row, Create Team button; right panel has Members and Log sub-tabs
- Switching from Year Group to Direct Users radio: year group panel hides, search box appears

- [ ] **Step 5: Commit**

```powershell
git add src/Tools/TeamsProvisioning.ps1
git commit -m "feat: Teams Provisioning tab skeleton with full XAML and UI wiring"
```

---

### Task 4: Async user load

**Files:**
- Modify: `src/Tools/TeamsProvisioning.ps1`
- Modify: `src/Demo.ps1`

- [ ] **Step 1: Append `Start-TpUserLoad` to TeamsProvisioning.ps1 (before `Initialize-TeamsProvisioningTool`)**
...
- [ ] **Step 2: Append `Start-TpUserLoadDemo` to Demo.ps1**
...
- [ ] **Step 3: Launch in Demo mode and verify**

Run: `Launch.cmd`, click Demo, click the Teams Provisioning tab.

Expected:
- Log sub-tab shows "Loaded 55 users (demo — Contoso Academy)."
- Year Group dropdown populated with Year 10, Year 11, Year 12, Staff
- Load Students button enabled

- [ ] **Step 4: Commit**

```powershell
git add src/Tools/TeamsProvisioning.ps1 src/Demo.ps1
git commit -m "feat: async user load for Teams Provisioning tab"
```

---

### Task 5: Team creation async

**Files:**
- Modify: `src/Tools/TeamsProvisioning.ps1`
- Modify: `src/Demo.ps1`

- [ ] **Step 1: Append `Start-TpCreateTeam` to TeamsProvisioning.ps1 (before `Initialize-TeamsProvisioningTool`)**
...
function Start-TpCreateTeam {
    ...
    Write-TpLog "Creating $template team: '$teamName'  ($($memberSnap.Count) members)" 'Text'
...
- [ ] **Step 2: Append `Start-TpCreateDemo` to Demo.ps1**
...
- [ ] **Step 3: Launch in Demo mode and test the full end-to-end flow**

Run: `Launch.cmd`, click Demo, switch to Teams Provisioning.

**Year group flow:**
1. Type "Maths Year 10" in the Team Name field
2. Leave Class Team selected
3. Select "Year 10" in dropdown, click Load Students → ~15 rows appear, all Owner unchecked
4. Click one row to select it, then click the Owner checkbox (second click) → checkbox ticks
5. Click Create Team
6. Expected: Log shows creation messages; after ~3 s stats panel shows "Team Created / Added 15 / Failed 0"
7. Create Team button re-enables

**Direct users flow:**
1. Switch to Direct Users radio → grid clears
2. Type "jake" in search → "Jake Morrison (jake.morrison@contoso.sch.uk)" appears
3. Double-click Jake → row added to grid, Create Team button enables (name still set)
4. Type "sarah" → Sarah Nkosi appears; double-click → second row added
5. Tick Owner on Sarah
6. Click Create Team → demo runs, stats show 2 added

- [ ] **Step 4: Commit**

```powershell
git add src/Tools/TeamsProvisioning.ps1 src/Demo.ps1
git commit -m "feat: team creation async with provisioning poll and member assignment"
```
