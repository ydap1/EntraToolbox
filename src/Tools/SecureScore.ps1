...
function Set-SsDisplay {
    ...
    try {
        $dt = [datetime]::Parse($Score.createdDateTime, [System.Globalization.CultureInfo]::InvariantCulture)
    ...
    for ($i = 0; $i -lt $bandKeys.Length; $i++) {
        $colorKey = if ($i -eq $activeBandIdx) { 'Text' } else { 'Muted' }
        $Script:SS_UI[$bandKeys[$i]].Foreground = New-SolidBrush $colorKey
    }
